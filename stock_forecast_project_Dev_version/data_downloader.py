...
def download_stock_data(ticker, start_date, end_date):
    ...
    try:
        ...
        # Remove period='1d' as start and end dates are specified
        ...
        # Add more specific log message about the CSV file's purpose
        logging.info(f"Saving original downloaded data for {ticker} to {file_path} for archival/debugging purposes.")
        historical_data.to_csv(file_path, index=False)
        logging.info(f"Successfully downloaded stock data for {ticker}.") # General success message
        return historical_data
    except Exception as e:
        logging.error(f"An error occurred while downloading stock data for {ticker}: {e}", exc_info=True)
        return pd.DataFrame()
